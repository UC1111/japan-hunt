...
- Pokémon Centerが403の場合 → Jina Readerで読み取りを試行
- Pokémon Centerが一時的に取得不能 → SQLiteの前回商品データで継続
- まだ自動投稿しない
...
### Pokémon Center取得について

GitHub Actionsからの直接アクセスが403になる場合があるため、Jina Readerの読み取りプロキシをフォールバックとして利用します。プロキシが失敗した場合は、SQLiteに保存済みの商品データを使って継続します。
